...
####                     DE genes                        ####
#############################################################
output <- fread("CD8_SeverevsMild.txt", sep="\t")
...
####                     Severe vs Mild                  ####
#############################################################
group_index <- which(metadata$group == "Mild" | metadata$group == "Severe")
#### test ####
covid_severe_index <- which(metadata$group == "Severe")
covid_mild_index <- which(metadata$group == "Mild")
set.seed(123)
covid_severe_index_test <- sample(covid_severe_index, round(length(covid_severe_index) / 5, 0), replace = F)
covid_mild_index_test <- sample(covid_mild_index, round(length(covid_mild_index) / 5, 0), replace = F)
dat_test <- dat[c(covid_severe_index_test, covid_mild_index_test), ]
metadata_test <- metadata[c(covid_severe_index_test, covid_mild_index_test), ]
metadata_test$group_id <- c(rep(1, length(covid_severe_index_test)), rep(0, length(covid_mild_index_test)))

covid_severe_index_training <- covid_severe_index[!(covid_severe_index %in% covid_severe_index_test)]
covid_mild_index_training <- covid_mild_index[!(covid_mild_index %in% covid_mild_index_test)]
dat_training <- dat[c(covid_severe_index_training, covid_mild_index_training), ]
metadata_training <- metadata[c(covid_severe_index_training, covid_mild_index_training), ]
metadata_training$group_id <- c(rep(1, length(covid_severe_index_training)), rep(0, length(covid_mild_index_training)))
...
id_group <- unique(metadata[,c("orig.donor", "group_id")])
...
for (i in 1:nrow(id_group)) {
    ## data pre-process
    test_index <- which(metadata$orig.donor == id_group[i, "orig.donor"])
...
file = "CD8_MLP_severe_mild_results_id.RData")