...
####                     DE genes                        ####
#############################################################
output <- fread("CD8_MildvsHC.txt", sep="\t")
...
####                    Mild vs Healthy                  ####
#############################################################
group_index <- which(metadata$group == "Mild" | metadata$group == "Healthy")
#### test ####
healthy_index <- which(metadata$group == "Healthy")
covid_index <- which(metadata$group == "Mild")
set.seed(123)
...
file = "CD8_MLP_mild_results_id.RData")