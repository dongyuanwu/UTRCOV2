...
mat0 <- readRDS("./CD8network/mat0.rds")
...
ego_bp <- kklist <- vector(mode="list", length=11)
...
# healthy module 2: no information
# healthy module 3
ego_bp[[2]] <- enrichGO(gene = hmod3,
                        keyType = "SYMBOL",
                        OrgDb = org.Hs.eg.db, 
                        ont = "BP", 
                        pAdjustMethod = "fdr", 
...
geneid <- bitr(hmod5, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
...
ego_bp[[2]]$module <- kklist[[2]]$module <- "Healthy Module 3"
ego_bp[[3]]$module <- kklist[[3]]$module <- "Healthy Module 4"
ego_bp[[4]]$module <- kklist[[4]]$module <- "Healthy Module 5"
ego_bp[[5]]$module <- kklist[[5]]$module <- "Healthy Module 6"
ego_bp[[1]]$GeneRatio <- ego_bp[[1]]$Count / length(hmod1)
ego_bp[[2]]$GeneRatio <- ego_bp[[2]]$Count / length(hmod3)
ego_bp[[3]]$GeneRatio <- ego_bp[[3]]$Count / length(hmod4)
ego_bp[[4]]$GeneRatio <- ego_bp[[4]]$Count / length(hmod5)
ego_bp[[5]]$GeneRatio <- ego_bp[[5]]$Count / length(hmod6)
kklist[[1]]$GeneRatio <- kklist[[1]]$Count / length(hmod1)
kklist[[2]]$GeneRatio <- kklist[[2]]$Count / length(hmod3)
kklist[[3]]$GeneRatio <- kklist[[3]]$Count / length(hmod4)
kklist[[4]]$GeneRatio <- kklist[[4]]$Count / length(hmod5)
...
bp1 <- dotplot(ego_bp[[6]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))
...
kklist[[6]] <- enrichKEGG(gene         = geneid$ENTREZID,
                          ...
                          pAdjustMethod= "fdr",
                          qvalueCutoff = 0.05)
kegg1 <- dotplot(kklist[[6]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))

# mild module 3
ego_bp[[7]] <- enrichGO(gene = mmod3,
                        keyType = "SYMBOL",
                        OrgDb = org.Hs.eg.db, 
                        ont = "BP", 
                        pAdjustMethod = "fdr", 
...
bp2 <- dotplot(ego_bp[[7]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))
...
kklist[[7]] <- enrichKEGG(gene         = geneid$ENTREZID,
                          ...
                          pAdjustMethod= "fdr",
                          qvalueCutoff = 0.05)
kegg2 <- dotplot(kklist[[7]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))


# mild module 4
ego_bp[[8]] <- enrichGO(gene = mmod4,
                        keyType = "SYMBOL",
                        OrgDb = org.Hs.eg.db, 
                        ont = "BP", 
                        pAdjustMethod = "fdr", 
...
bp3 <- dotplot(ego_bp[[8]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))
...
kklist[[8]] <- enrichKEGG(gene         = geneid$ENTREZID,
                          ...
                          pAdjustMethod= "fdr",
                          qvalueCutoff = 0.05)
kegg3 <- dotplot(kklist[[8]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))
...
ego_bp[[7]]$module <- kklist[[7]]$module <- "Mild Module 3"
ego_bp[[8]]$module <- kklist[[8]]$module <- "Mild Module 4"
ego_bp[[6]]$GeneRatio <- ego_bp[[6]]$Count / length(mmod1)
ego_bp[[7]]$GeneRatio <- ego_bp[[7]]$Count / length(mmod3)
ego_bp[[8]]$GeneRatio <- ego_bp[[8]]$Count / length(mmod4)
kklist[[6]]$GeneRatio <- kklist[[6]]$Count / length(mmod1)
kklist[[7]]$GeneRatio <- kklist[[7]]$Count / length(mmod3)
kklist[[8]]$GeneRatio <- kklist[[8]]$Count / length(mmod4)
...
bp1 <- dotplot(ego_bp[[9]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))
...
kklist[[9]] <- enrichKEGG(gene         = geneid$ENTREZID,
                          ...
                          pAdjustMethod= "fdr",
                          qvalueCutoff = 0.05)
kegg1 <- dotplot(kklist[[9]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))

# severe module 2
ego_bp[[10]] <- enrichGO(gene = smod2,
                         keyType = "SYMBOL",
                         OrgDb = org.Hs.eg.db, 
                         ont = "BP", 
                         pAdjustMethod = "fdr", 
...
bp2 <- dotplot(ego_bp[[10]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))

geneid <- bitr(smod2, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
kklist[[10]] <- enrichKEGG(gene         = geneid$ENTREZID,
                           organism     = 'hsa',
                           pAdjustMethod= "fdr",
                           qvalueCutoff = 0.05)
kegg2 <- dotplot(kklist[[10]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))


# severe module 3
ego_bp[[11]] <- enrichGO(gene = smod3,
                         keyType = "SYMBOL",
                         OrgDb = org.Hs.eg.db, 
                         ont = "BP", 
                         pAdjustMethod = "fdr", 
                         qvalueCutoff = 0.05)
ego_bp[[11]] <- clusterProfiler::simplify(
    ego_bp[[11]],
    cutoff = 0.7,
    by = "p.adjust",
    select_fun = min,
...
bp3 <- dotplot(ego_bp[[11]], showCategory=10, font.size=15, title="Biological Process") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))

geneid <- bitr(smod3, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
kklist[[11]] <- enrichKEGG(gene         = geneid$ENTREZID,
                           organism     = 'hsa',
                           pAdjustMethod= "fdr",
                           qvalueCutoff = 0.05)
kegg3 <- dotplot(kklist[[11]], showCategory=10, font.size=15, title="KEGG Pathway") +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=15))

ego_bp[[9]] <- na.omit(as.data.frame(ego_bp[[9]])[, -7])
ego_bp[[10]] <- na.omit(as.data.frame(ego_bp[[10]])[, -7])
ego_bp[[11]] <- na.omit(as.data.frame(ego_bp[[11]])[, -7])
kklist[[9]] <- na.omit(as.data.frame(kklist[[9]])[, -7])
kklist[[10]] <- na.omit(as.data.frame(kklist[[10]])[, -7])
kklist[[11]] <- na.omit(as.data.frame(kklist[[11]])[, -7])

ego_bp[[9]]$module <- kklist[[9]]$module <- "Severe Module 1"
ego_bp[[10]]$module <- kklist[[10]]$module <- "Severe Module 2"
ego_bp[[11]]$module <- kklist[[11]]$module <- "Severe Module 3"

ego_bp[[9]]$GeneRatio <- ego_bp[[9]]$Count / length(smod1)
ego_bp[[10]]$GeneRatio <- ego_bp[[10]]$Count / length(smod2)
ego_bp[[11]]$GeneRatio <- ego_bp[[11]]$Count / length(smod3)
kklist[[9]]$GeneRatio <- kklist[[9]]$Count / length(smod1)
kklist[[10]]$GeneRatio <- kklist[[10]]$Count / length(smod2)
kklist[[11]]$GeneRatio <- kklist[[11]]$Count / length(smod3)
...
ggplot(bp, aes(x = GeneRatio, y = Description)) + 
    ...
    ylab(NULL) +
    ggtitle("GO Biological Process for CD8+ T Cells") + facet_wrap(.~module, ncol=6) +
...
ggplot(kk, aes(x = GeneRatio, y = Description)) + 
    ...
    scale_colour_gradient(limits=c(0, 0.05), low="red") +
    ylab(NULL) +
    ggtitle("KEGG Pathway for CD8+ T Cells") + facet_wrap(.~module, ncol=6) +
    theme(plot.title = element_text(hjust = 0.5), text=element_text(size=20)) +
