...
####                     DE genes                        ####
#############################################################
output <- fread("CD8_SeverevsHC.txt", sep="\t")
...
####                    Severe vs Healthy                ####
#############################################################
group_index <- which(metadata$group == "Severe" | metadata$group == "Healthy")
#### test ####
healthy_index <- which(metadata$group == "Healthy")
covid_index <- which(metadata$group == "Severe")
set.seed(123)
...
file = "CD8_MLP_severe_results_id.RData")