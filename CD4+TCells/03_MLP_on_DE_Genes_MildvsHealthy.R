...
####                     DE genes                        ####
#############################################################
output <- fread("CD4_MildvsHC.txt", sep="\t")
...
####                    Mild vs Healthy                  ####
#############################################################
group_index <- which(metadata$group == "No" | metadata$group == "Healthy")
#### test ####
healthy_index <- which(metadata$group == "Healthy")
covid_index <- which(metadata$group == "No")
set.seed(123)
...
file = "CD4_MLP_mild_results_id.RData")