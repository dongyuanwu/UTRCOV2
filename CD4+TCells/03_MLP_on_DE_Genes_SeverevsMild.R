...
####                     DE genes                        ####
#############################################################
output <- fread("CD4_SeverevsMild.txt", sep="\t")
...
####                     Severe vs Mild                  ####
#############################################################
group_index <- which(metadata$group == "No" | metadata$group == "Yes")
#### test ####
covid_severe_index <- which(metadata$group == "Yes")
covid_mild_index <- which(metadata$group == "No")
set.seed(123)
...
#### sensitivity ####
sensitivity_t <- function (table) {
    return(table[2, 2] / (table[2, 2] + table[2, 1]))
}

#### specificity ####
specificity_t <- function (table) {
...
file = "CD4_MLP_severe_mild_results_id.RData")